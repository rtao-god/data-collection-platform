...
class SdkProcessingGateway:
    """Maps trusted processing work to the capability-scoped Worker Gateway protocol."""
    ...
    def _required_build_identity(self) -> str:
        if self._build_identity is None:
            raise RuntimeError("processing worker must register before processing work")
