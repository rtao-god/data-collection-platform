...
blockers into an atomically sealed `config` record. Child rows are inserted before the root under
a digest-scoped transaction lock; root insertion validates the complete composition and closes it
to later mutation. The database does not store bundle bytes or create an object-store reference.
Runtime persistence remains blocked until content-addressed upload and integrity verification
exist.
