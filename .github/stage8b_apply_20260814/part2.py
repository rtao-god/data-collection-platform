...
class PostgresReviewRepository:
    ...
    def _require_candidate_revision(
        ...
        *,
        ...
        suffix = " FOR SHARE" if for_update else ""
    ...
    def _insert_decision(
        ...
        connection.execute(
            sa.text(
                ...
                sa.bindparam("evidence_references", type_=sa.ARRAY(sa.Text()))
            ),
            {
                **decision.model_dump(),
...
def _suppression(row: RowMapping) -> SuppressionRevision:
    scopes = tuple(
        ...
        for scope, enabled in (
            ("discovery", row["suppress_discovery"]),
            ("export", row["suppress_export"]),
            ("normalization", row["suppress_normalization"]),
        )
        if enabled
    )
...
''',
    ...
    import_line = (
        "from collection_infrastructure.postgres.review_repository import "
