...
def apply(root: Path) -> None:
    artifact_metadata = root / (
        "packages/collection_infrastructure/src/collection_infrastructure/postgres/"
        "artifact_metadata.py"
    )
    text = artifact_metadata.read_text(encoding="utf-8")
    if "artifact_cleanup_tombstones = sa.Table(" not in text:
        insertion = '''
...
'''
        ...
        if marker not in text:
            raise RuntimeError("artifact metadata table inventory anchor is missing")
        ...
        artifact_metadata.write_text(text, encoding="utf-8")

    _write(
        root / "packages/collection_infrastructure/src/collection_infrastructure/postgres/artifact_cleanup.py",
        '''from __future__ import annotations
        ...
        with self._engine.begin() as connection:
            ...
            if remaining > 0:
                artifact_record_exists = sa.exists(
                    sa.select(1).where(
                        artifact_records.c.upload_id == artifact_uploads.c.upload_id
                    )
                )
                ...
                candidates = (
                    connection.execute(
                        ...
                        .where(
                            ...
                            artifact_uploads.c.prepared_at_utc <= cutoff,  # noqa: SIM300
                ...
                for row in candidates:
                    ...
                    reason = (
                        "orphan_staging" if row["state"] == "prepared" else "orphan_verified"
                    )
                    connection.execute(
                        sa.insert(artifact_cleanup_tombstones).values(
                            ...
                            eligible_at_utc=_datetime(row, "prepared_at_utc")
                            + policy.grace_period,
                            claimed_at_utc=now_utc,
                            claim_expires_at_utc=claim_expires,
                            attempt_count=1,
